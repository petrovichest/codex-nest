...
его стили, компоненты и тесты. Общая компоновка сохраняется; согласованные
изменения общих правил вносятся вместе в код и этот документ.
...
| Цвет, текст, отступ | Общие токены по роли; радиусы `6 / 10 / 14px`, Onest и существующую плотность                          |
...
Не копируйте локальное исключение в новую фичу как общий стандарт.
...
- Иерархию создают отступы, положение, размер текста и тонкие границы. Тени
  обозначают перекрывающие поверхности; обычным строкам настроек тень не нужна.
...
| --radius-lg            | `14px`   | Композер, диалоги, крупные накладываемые панели  |
...
`10px` для обычных контролов, карточек и меню, `14px` для диалогов, композера и
крупных накладываемых панелей. Используйте `var(--radius-sm)`, `var(--radius-md)`
или `var(--radius-lg)`, а не
промежуточные `7/11/13px`. У мобильного sheet нижние углы остаются прямыми.
Круги, капсулы и перечисленные оптические исключения проверяет
...
Высота определяется содержимым, а между вариантами и действиями остаётся `16px`.
...
сохранение позиции чтения. [Dialog.test.tsx][dialog-tests] проверяет начальный
