...
зафиксировано ниже и применяется к композеру, области чата и боковой навигации. Остальные экраны
переводятся на него по отдельным задачам.
...
- Основу составляют нейтральные поверхности; боковая навигация использует фон
  страницы, а её плавающие элементы — мягкий тон чата. Основное действие контрастное
  и почти монохромное.
...
Для боковой навигации выбран открытый список и объём кнопок по наведению,
описанные ниже. Макеты настроек и остальных экранов ещё не выбраны.

#### Боковая навигация: открытый список и объём по наведению

Утверждено 18 сентября 2026. Панель сохраняет ширину, порядок действий, режимы
«Проекты / Активные», закреплённые сессии, архив и плотность списка. Её фон совпадает
с `--color-canvas`, вертикальной границы нет. Общие подложки под тремя верхними
строками и переключателем разделов отсутствуют; обе группы имеют одинаковую ширину
и поля `8px` с учётом боковых safe area на мобильном экране.

- Верхние кнопки имеют высоту `38px`, кнопки переключателя — `34px`. Размеры действий
  в строках сохраняются, включая области касания `44px` для меню сессии.
- Обычные кнопки в покое прозрачны. Наведение мышью и `focus-visible` проявляют
  капсулу с мягкой тенью; нажатие использует мягкую заливку и внутреннюю тень.
  Геометрия не меняется, клавиатурный контур сохраняется. На touch нет залипающего
  hover, `prefers-reduced-motion` отключает переходы.
- Выбранный раздел остаётся мягко заполненной капсулой без общей подложки группы.
  Выбранная сессия и строка под указателем используют поверхность и тень чата
  со скруглением `18px`. Подложка не добавляется каждой строке в покое.
- В дереве проектов подложка сессии расширена влево на `8px` внутрь существующего
  отступа: до названия остаётся `12px`, положение и ширина текста не меняются.
  В активном списке отступ до названия тоже `12px`.
- Индикатор статуса имеет размер `3 × 20px`, расположен по центру высоты и отстоит
  от правого края на `10px`. Между ним и меню действий — минимум `6px`; резерв
  текста учитывает раскрытые действия. Пульсация не обрезается ради скруглений.
- Завершение, подтверждение, ошибка, доступное обновление и соединение сохраняют
  смысловые цвета. Раскрытие, перетаскивание, навигация и мобильные жесты не меняются.

Оформление ограничено `.sidebar`. Локальные `--sidebar-surface`,
`--sidebar-control-active`, `--sidebar-surface-shadow`, `--sidebar-control-shadow`
и `--sidebar-pressed-shadow` используют палитру и объём чата. Геометрия задаётся
`--sidebar-radius-row: 18px` и `--sidebar-radius-menu: 20px`; размеры остальных
экранов и общие токены не меняются. Мобильный drawer остаётся непрозрачным.

<p>
  <a href="../apps/client/e2e/visual/__screenshots__/chromium/sidebar-hover-light.png"><img src="../apps/client/e2e/visual/__screenshots__/chromium/sidebar-hover-light.png" width="292" alt="Светлая навигация: открытый список и объём кнопки по наведению" /></a>
  <a href="../apps/client/e2e/visual/__screenshots__/chromium/sidebar-hover-dark.png"><img src="../apps/client/e2e/visual/__screenshots__/chromium/sidebar-hover-dark.png" width="292" alt="Тёмная навигация: переключатель без общей подложки и мягкое выделение сессии" /></a>
</p>
...
достигает `840px`, сообщения — `824px`. Панели выступают на `8px` с каждой стороны.
...
и создание ветки. Композер сохраняет прежние стили и геометрию; оформление боковой
колонки описано отдельно выше.
...
Подпись статуса и иконка образуют компактную группу высотой `32px`:
внутренние поля слева и справа `12px`, промежуток между иконкой и текстом `8px`.
Эта геометрия одинакова у выполнения, завершения, ошибки и прерывания, а также
у статуса без доступного журнала. Текущие размеры текста и цвета состояний сохраняются.

В покое фон прозрачен. Если журнал доступен, наведение и нажатие используют
общие состояния кнопок чата, а фокус обозначается видимым контуром. Область нажатия
ограничена подписью с её внутренними полями. Таймер текущей работы остаётся рядом
с промежутком `8px`, длинная подпись сокращается, не выталкивая время за экран.

#### Таблицы в чате — вариант A
...
<p>
  <a href="assets/chat-tables-1440-light.png"><img src="assets/chat-tables-1440-light.png" width="340" alt="Светлая тема: мягкие таблицы и отступы статуса на компьютере" /></a>
  <a href="assets/chat-tables-1440-dark.png"><img src="assets/chat-tables-1440-dark.png" width="340" alt="Тёмная тема: таблицы без жёсткой сетки" /></a>
  <a href="assets/chat-tables-390-light.png"><img src="assets/chat-tables-390-light.png" width="195" alt="Светлая тема на телефоне: короткая таблица помещается, широкая прокручивается" /></a>
  <a href="assets/chat-tables-390-dark.png"><img src="assets/chat-tables-390-dark.png" width="195" alt="Тёмная тема на телефоне: таблицы и статус завершения" /></a>
</p>

Снимки получаются из `table-status.spec.ts` с `NODE_ENV=test`,
`UPDATE_TABLE_DOCS=1` и фильтром `soft tables (390|1440)px (light|dark) ru`.

#### Редактор сообщения в очереди — вариант A
...
- Поля карточки — `16px 20px`, на телефоне — `16px`. Текст — Onest `16px/1.6`, вес
...
- Через `12px` после поля справа расположены «Отмена» и «Сохранить»: высота `36px`,
  горизонтальные поля `18px`, промежуток `8px`, текст `14px`. Сохранение выделено
  общей мягкой подложкой основных действий чата.
...
В ответах и планах агента все изображения собираются **под текстом своего сообщения**.
...
<p>
  <a href="assets/chat-gallery-1440-light.png"><img src="assets/chat-gallery-1440-light.png" width="340" alt="Галерея на компьютере, светлая тема: ссылки и компактная подборка под ответом" /></a>
  <a href="assets/chat-gallery-1440-dark.png"><img src="assets/chat-gallery-1440-dark.png" width="340" alt="Галерея на компьютере, тёмная тема" /></a>
  <a href="assets/chat-gallery-390-light.png"><img src="assets/chat-gallery-390-light.png" width="195" alt="Галерея на телефоне, светлая тема" /></a>
  <a href="assets/chat-gallery-390-dark.png"><img src="assets/chat-gallery-390-dark.png" width="195" alt="Галерея на телефоне, тёмная тема" /></a>
</p>

Снимки воспроизводятся из сценариев `chat-media.spec.ts`: установить `NODE_ENV=test`
и `UPDATE_GALLERY_DOCS=1`, запустить Chromium с фильтром `image gallery at (390|1440)`.
...
ширина — `min(840px, 100% − 40px)`, на `16px` больше колонки сообщений.
Обе панели расширены на `4px` с каждой стороны относительно прежней раскладки,
а сообщения сужены на `4px`; выступ сохраняется при открытых сведениях.
Название задачи и проект используют
...
| --color-sidebar        | `#eef0e9` | `#20231f` | Прежний фон навигации; текущая использует canvas   |
...
на экранах шириной до `359px`. Ввод в композере имеет размер `16px`.
...
модульные тесты, сборку и отдельную проверку типов сервера. Модульный тест радиусов
