...
В режиме Plan последний план показывается один раз в конце обсуждения, над
композером и вместе с кнопками запуска. Старые версии остаются в истории;
при выходе из Plan последняя версия тоже возвращается на своё место в истории.
...
`12px` (`8px` при ширине до `359px` включительно). Ввод в композере имеет размер `16px`.
