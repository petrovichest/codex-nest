...
Единого глобального токена поверхности пока нет: эталон цвета — существующий
`--sidebar-panel` в [styles.css][styles]. При реализации перехода существующие
локальные переменные должны разрешаться в этот единый тон в обеих темах.
Документ не вводит новую библиотеку компонентов и не объявляет старые локальные
значения уже обновлёнными.
...
Композер — основное поле ввода приложения. Его реализованная геометрия и сценарии
сохраняются; поверхность и состояния контролов подчиняются общим правилам выше.
Локальные `--composer-*` определены в [styles.css][styles] и ещё ожидают унификации
цвета тёмной темы и объёма выбранных/наведённых кнопок.
...
| Радиус          | `30px` на desktop, `24px` до `820px` включительно              |
...
тоном левой панели (`#f8f9f6` / `#242722`). Текущий `--chat-user` ещё требует
унификации, указанной в списке перехода. Не вводить для вставки отдельный оттенок.
...
| Композер                                                | `30px` desktop / `24px` mobile                    | Существующая геометрия композера              |
...
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Тон поверхностей          | Привести `--sidebar-surface` в тёмной теме, `--composer-surface`, поверхности чата, пользовательские сообщения и вставленные карточки к единому тону `--sidebar-panel` |
| Кнопки вне навигации      | Заменить постоянную заливку главных действий и тональные hover-состояния объёмом по общей таблице; сохранить анимацию нажатия                                          |
