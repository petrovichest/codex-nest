...
До `820px` шапка располагается поверх ленты. Её фон непрозрачен, тень и цвет
совпадают с другими мягкими поверхностями. Область прокрутки начинается у нижней
границы системного безопасного отступа, **выше нижнего края шапки**: карточки
не обрезаются прямой линией под панелью и видны вокруг её закруглений.
...
Начальный отступ ленты оставляет `22px` под шапкой и прокручивается вместе с
историей. `scroll-padding-top` сохраняет `8px` между панелью и целью перехода
или фокуса. Меню ответвлений открывается на `8px` ниже панели; сведения,
навигация и диалоги сохраняют свои верхние слои. На компьютере шапка остаётся
над областью прокрутки. Настройки и экран подготовки ответвления используют
свою прежнюю компоновку.
