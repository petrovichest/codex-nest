...
Шкала приложения — **12 / 14 / 16 / 18 / 20px**, одинаковая на desktop и mobile.
...
| Блоки кода, технические журналы, таблицы                                                 | `--text-small`                   | `14px`; встроенный код наследует размер окружающего текста |
...
Основной текст `16px` действует во всём клиенте: в навигации и переключателе
...
существующего заголовка экрана подключения. Обычные подписи не уменьшаются ниже
`12px`. Не добавляйте локальные размеры `13/15/17/19/22px`, дробные размеры или
...
Сообщение сохраняет размер `16px / 24px` на всём пути: ввод, очередь,
...
высоты нельзя. Пустой композер сохраняет `94px` на desktop и `86px` до `820px`.
...
проверяет `typography.spec.ts` в Chromium и WebKit, на русском и английском,
