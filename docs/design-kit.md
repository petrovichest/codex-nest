...
Панель сведений текущей и новой сессии использует тот же уровень объёма, что и
боковая навигация: `--sidebar-panel-shadow`. Вложенные выбранные вкладки и кнопки
сохраняют тень `--sidebar-surface-shadow`. На desktop панель располагается выше
фона соседней области чата, чтобы он не обрезал внешнюю тень. При проверке нужен
снимок с пространством вокруг панели: совпадения CSS-токенов недостаточно.
При ширине до `1279px` включительно
панель открывается поверх чата; фон за ней размывается на `8px` без затемнения,
как у мобильного списка сессий. Подложка закрытия остаётся прозрачной и без тени
при наведении, фокусе и нажатии; клик по ней закрывает сведения.
...
До `820px` включительно шапка настроек повторяет геометрию шапки чата: одна
закреплённая капсула с отступом `8px` плюс боковые системные области. В ней слева
кнопка боковой панели, справа — горизонтально прокручиваемые разделы. У краёв
ленты разделы плавно исчезают, как сессии в боковой панели. Базовая высота шапки —
`44px`; увеличенный шрифт увеличивает её высоту. Заголовок остаётся доступным
семантически, но не занимает место в мобильной капсуле.

Карточки отступают от краёв на `16px` плюс боковые системные области. Прокрутка
идёт за шапкой без прямоугольной подложки под ней; её начальный отступ оставляет
`22px` после шапки и прокручивается вместе с содержимым. Шапка и вкладки остаются
на месте. Нижняя системная область находится за пределами прокрутки; после последней
карточки остаётся `24px`. Внутри карточки последнее поле также отстоит от нижнего
края на `24px`, без дополнительного отступа последней строки.
...
#### Отправленные ответы на вопросы

Карточка `userInputResponse` сохраняет порядок «тема → полный вопрос → ответ».
Все вопросы видны сразу. Одна общая поверхность использует `--color-floating`
и `--chat-shadow`; отдельные строки не получают карточек или разделительных линий.
Ширина, выравнивание сообщения и общий footer с временем и копированием сохраняются.

- Тема: `--text-small` (`14px / 21px`), вес `400`, `--muted`.
- Полный вопрос: `--text-message` (`15px / 22.5px`), вес `400`, `--muted-strong`.
- Ответ: `--text-message` (`15px / 22.5px`), вес `500`, `--text-strong`.
- Между темой и вопросом — `8px`, перед ответом — `10px`, между группами — `24px`.
  Несколько ответов на один вопрос разделяются отступом `8px`.
- Поля карточки — `22px`, радиус — `28px`; до `820px` включительно — `16px` и `24px`.
  Типографика на телефоне не уменьшается. Длинный текст переносится, переносы строк
  в вопросах и свободных ответах сохраняются.
- Конечный суффикс ` (Recommended)` скрывается только в отображаемом ответе.
  Исходные данные, отправка агенту и копирование сохраняют полный текст.
...
- В просмотрщике изображение увеличивается жестом двумя пальцами на сенсорном
  экране и тачпаде, Ctrl + колёсиком или клавишами `+` и `−`; `0` сбрасывает масштаб.
  Двойной тап переключает между исходным видом и увеличением в 3 раза. Увеличенное
  изображение можно перемещать, масштаб ограничен 6× и сбрасывается при перелистывании.
...
Документ открывается в `ArtifactViewer` из вкладки «Артефакты» или по ссылке
в сообщении. Markdown расположен прямо на общей поверхности `--color-floating`,
без внутренней рамки, дополнительного листа и растягивания по высоте панели.
Поля содержимого — `24px`, до `820px` включительно — `20px` с учётом safe area.
Размеры панели и существующие режимы desktop / overlay / mobile сохраняются.

Шапка неподвижна при прокрутке документа. Имя файла использует `--text-ui`,
формат и размер — `--text-caption`; длинное имя сокращается и доступно через `title`.
Иконка файла не получает отдельной подложки. Круглые действия «Обновить», «Скачать»
и «Вернуться / Закрыть» имеют размер `36px`, при `(pointer: coarse)` — `44px`,
и общие состояния кнопок. Минимальная высота шапки — `60px` плюс верхняя safe area
на телефоне.

Основной текст — `--text-message` с сохранением относительной иерархии Markdown
и пользовательских размеров шрифта. Таблицы используют оформление таблиц чата,
включая область `.markdown-table-scroll` с локализованным именем, клавиатурным
фокусом и собственной горизонтальной прокруткой. Блоки кода имеют радиус `20px`
и `--chat-inner-shadow`. Встроенный код наследует размер текста, блоки кода и
таблицы используют `--text-code`. Ссылки подчёркнуты.

Оформление содержимого других форматов не меняется. Проверки тем, адаптивности,
safe area, длинных документов и клавиатуры — `artifact-viewer.spec.ts`.
...
панели; сведения, навигация и диалоги сохраняют свои верхние слои. Настройки
используют ту же геометрию шапки, но собственные разделы внутри неё; экран
подготовки ответвления сохраняет свою прежнюю компоновку.
...
Многострочные варианты выбора модели и уровня рассуждений используют геометрию
вложенной карточки: `--chat-radius-card` (`20px`), поля `--space-3` × `--space-4`
(`12px` сверху/снизу и `16px` по бокам), промежутки `--space-2` (`8px`). Текст
выровнен сверху, галочка — по центру высоты. При наведении, фокусе и выборе
сохраняются общие тон и тень, при нажатии — продавливание; геометрия не меняется.
Меню начинается со строки «Модель» с кнопкой закрытия справа, без отдельной
видимой шапки. Доступное имя диалога «Настройки модели» сохраняется.
...
До подключения применяется тема системы, если пользователь ещё не выбирал тему,
или сохранённая настройка. Ручной выбор остаётся в настройках. Стартовая тема
применяется до отрисовки внешним same-origin скриптом, совместимым с CSP сервера;
дальнейшая смена темы и системные панели Android сопровождаются на уровне корня
приложения, включая загрузку, подключение и смену сервера.

Форма — одна парящая карточка шириной до `430px`, с радиусом `28px` и общей тенью
поверхности. Внутренние поля — `32px`, до `820px` — `24px`. Внешние поля экрана —
`32px` и `16px` соответственно плюс safe area, учтённая только снаружи карточки.
При достаточной высоте карточка центрируется; при нехватке места экран прокручивается
от верхнего отступа, сохраняя доступ к полям и кнопке при открытой клавиатуре.

Между заголовком и описанием — `8px`, перед полями — `24px`, между полями,
уведомлениями и действием — `16px`; между подписью и вводом — `8px`.
Поля используют общий мягкий фокус без жёсткого контура. Предупреждение HTTP и ошибка
сохраняют смысловые цвета, используют радиус `20px`, поля `12px × 16px` и компактную
тень без рамки. Главное действие прозрачно в покое; ожидание использует `ActionLabel`.
