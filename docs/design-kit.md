...
Это принятый стандарт, а не описание уже завершённого редизайна всех экранов.
[Согласованные превью](#согласованные-превью) показывают цель; оставшаяся работа
перечислена в [списке перехода](#что-ещё-перевести-на-новый-дизайн).
Обновление документа само по себе не меняет компоненты приложения.
...
- **Визуальные правила и превью** описывают принятый стандарт. Пометка «текущая
  ...
  При несовпадении старых CSS со стандартом ориентируйтесь на список перехода.
...
точный тон прежнего `--sidebar-panel`; подложка навигации, композер и его меню,
вложения, вставленные карточки и сообщения пользователя уже используют его.
Остальные поверхности переводятся отдельно по списку перехода. Локальные имена
...
Сильная тень общей навигации не переносится на каждую карточку. Фокус сохраняет
дополнительный видимый индикатор; одной тени недостаточно для доступности.
...
- Обе верхние группы имеют одинаковую ширину и внутренние поля `8px`.
  ...
  При ширине самой панели до `260px` переключатель сохраняет текст `16px`:
...
селектор hover/focus/disabled/active. Единый тон распространяется также на
`--sidebar-surface`: его оставшееся отличие в тёмной теме входит в список перехода.
...
Фокус усиливает понятность активного ввода и сохраняет видимый доступный индикатор.
...
вариант: пустое поле и одна строка занимают `94px` на desktop и `86px` до `820px`
включительно. Ширина, положение и набор действий сохранены. Текст — Onest `16px/24px`
...
<p>
  <a href="assets/composer-compact-desktop-light.png"><img src="assets/composer-compact-desktop-light.png" width="420" alt="Компактный светлый композер: 94px, единый тон со вставленной карточкой" /></a>
...
<p>
  <a href="assets/composer-compact-mobile-light.png"><img src="assets/composer-compact-mobile-light.png" width="260" alt="Светлый мобильный композер: пустое поле высотой 86px" /></a>
...
- Поля карточки — `16px 20px`, на телефоне — `16px`. Текст — Onest `16px/24px`, вес
...
- Через `12px` после поля справа расположены «Отмена» и «Сохранить»: высота `36px`,
  горизонтальные поля `18px`, промежуток `8px`, текст `16px`. Обе кнопки прозрачны
...
`8px`. Верхний край совпадает с подложкой навигации. На компьютере шапка
центрирована по видимому пузырю инпута:
ширина — `min(840px, 100% − 40px)`, на `16px` больше колонки сообщений.
Выступ относительно колонки сообщений сохраняется при открытых сведениях.
Название задачи использует Onest `16px/20px`, проект — `14px/20px`, вес обоих
...
строки: название `16px/20px`, проект `14px/18px`.
...
Скачивание в списке артефактов — круглая кнопка `32 × 32px` без разделителя.
...
новой фичи. Унификация оставшихся цветов перечислена в списке перехода.
...
Стандартная шкала приложения — **12 / 14 / 16 / 18 / 20px**, одинаковая на desktop и mobile.
...
| Основной UI, названия проектов и сессий, обычные кнопки, меню, вкладки, подписи полей    | `--text-ui`, `--text-label`      | `16px`; базовый интервал `24px`                            |
| Заголовок сессии в верхней панели чата                                                   | `--text-ui`                      | `16px / 20px` на всех ширинах                              |
...
| Сообщения, основной ввод, редакторы очереди и вставленного текста, обычные поля и select | `--text-message`, `--text-input` | `16px / 24px`                                              |
| Заголовки экранов, разделов настроек и карточек                                          | `--text-section`                 | `18px`; заголовки экранов и разделов — интервал `24px`     |
| Заголовки диалогов и пустых состояний                                                    | `--text-dialog`                  | `20px`; заголовок диалога — интервал `28px`                |

По умолчанию основной текст `16px` действует во всём клиенте: в навигации и переключателе
...
Два ограниченных исключения: `--text-micro` / `--text-tiny` (`10px`) для коротких
счётчиков, индикаторов и обозначений типа файла; `--text-display` (`24px`) для
существующего заголовка экрана подключения. По умолчанию обычные подписи не уменьшаются ниже
`12px`. Не добавляйте в стили компонентов локальные размеры `13/15/17/19/22px`, дробные размеры или
отдельную мобильную шкалу. Иерархия заголовков внутри пользовательского Markdown
сохраняется и не подменяется заголовками интерфейса.

Сообщение сохраняет единый размер (по умолчанию `16px / 24px`) на всём пути: ввод, очередь,
редактирование, отправка. Обычные поля и select также имеют `16px` на компьютере
...
высоты нельзя. При стандартных размерах пустой композер сохраняет `94px` на desktop
и `86px` до `820px`. При увеличении шрифта его высота растёт вместе с текстом.
...
с шагом **1px**; рядом показан живой пример и отдельная кнопка сброса. Общий сброс
...
<p>
  <a href="assets/typography-desktop-chat.png"><img src="assets/typography-desktop-chat.png" width="420" alt="Чат на компьютере: единый текст сообщения и ввода 16px" /></a>
  <a href="assets/typography-desktop-settings.png"><img src="assets/typography-desktop-settings.png" width="420" alt="Настройки на компьютере: заголовки 18px, поля и подписи 16px, пояснения 14px" /></a>
</p>
...
<p>
  ...
  <a href="assets/typography-mobile-settings.png"><img src="assets/typography-mobile-settings.png" width="260" alt="Мобильные настройки с полями 16px" /></a>
</p>
...
## Что ещё перевести на новый дизайн

Документ фиксирует целевое оформление; обновление CSS и компонентов выполняется
отдельными задачами. Список перехода не задаёт альтернативных визуальных правил.

| Область                   | Оставшаяся работа                                                                                                                      |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Тон поверхностей          | Привести `--sidebar-surface` в тёмной теме, шапку, планы, вопросы и остальные нейтральные карточки чата к `--color-floating`           |
| Кнопки остальных экранов  | Перевести оставшиеся действия чата и настроек на объём по общей таблице; навигация, композер и его меню уже используют новые состояния |
| Поля, настройки и диалоги | Применить парящие поля и подложки групп; привести старые формы к существующим bubble-радиусам без изменения сценариев                  |
| Примитивы и расширение    | Перевести оставшиеся нейтральные поверхности и контролы на общие правила, сохранив компактность и доступность                          |

После изменения соответствующей области обновляйте список и браузерные проверки.
Не переносите старые цвета, радиусы и снимки в основные правила как второй стандарт.
