...
реализован вариант C, описанный ниже. Макеты навигации, настроек и остальных
экранов ещё не выбраны; порядок и сроки их переработки не определены.
...
- Первая правка мобильных отступов касалась поля ввода и пространства рядом с
  ним. Лента переработана отдельно в варианте C; готовый композер сохранён.
...
#### Область чата: вариант C

Пояснения, план, результат и журнал одного ответа образуют общую мягкую
поверхность. Новое сообщение пользователя, включая уточнение во время работы,
разделяет ответ на последовательные группы. Порядок сообщений сохраняется.
...
<p>
  <a href="assets/chat-soft-light.png"><img src="assets/chat-soft-light.png" width="340" alt="Вариант C в светлой теме: общий блок ответа, мягкая тень и скруглённые чекбоксы" /></a>
  <a href="assets/chat-soft-dark.png"><img src="assets/chat-soft-dark.png" width="340" alt="Вариант C в тёмной теме: поверхности чата совпадают по тону с композером" /></a>
</p>
...
<p>
  ...
  <a href="assets/chat-soft-mobile-dark.png"><img src="assets/chat-soft-mobile-dark.png" width="280" alt="Тёмная тема на 320px: вопросы и действия на общей мягкой поверхности" /></a>
</p>
...
- Поля общего блока — `22px` на desktop и `16px` на mobile; промежуток между
  группами — `24px`. Шапка имеет высоту `64px`. Компактный триггер журнала
  сохраняет отдельный таймер и не растягивается на всю строку.
- Фон ответа рисуется отдельным декоративным элементом CSS Grid. Компоненты
  сообщений, вопросов и журнала не перегруппировываются в новые React-обёртки
  при уточнении пользователя: сохраняются черновик, выделение, фокус и раскрытие.
...
| `--chat-radius-compact`  | `24px`   | Шапка и мобильные поверхности                    |
| `--chat-radius-surface`  | `28px`   | Ответы, сообщения пользователя и desktop-диалоги |
...
оформления — композер и вариант C выше. Исторические снимки помогают найти
...
На мобильном заголовок равен `56px + --app-safe-area-top`, горизонтальные поля
ленты — `12px`, композера — `8px`. Ввод в композере имеет размер `16px`.
