...
- Поля акцентных карточек — `22px` на desktop и `16px` на mobile. Обычные сообщения
  выровнены по полям ленты; промежуток между сообщениями — `24px`. Шапка имеет
  высоту `64px`. Компактный триггер журнала сохраняет отдельный таймер
  и не растягивается на всю строку.
