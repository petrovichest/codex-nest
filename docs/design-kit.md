...
- Основу составляют нейтральные поверхности; боковая навигация собрана на общей
  плавающей подложке, а её выбранные элементы используют поверхность и тень чата. Основное действие контрастное
  и почти монохромное.
...
Для боковой навигации выбрана парящая панель с единым объёмом выбранных элементов,
...
- Внешние поля на компьютере — `20px`; занятое навигацией место — `332px`.
...
- На мобильных внешние поля — `8px` плюс safe area, учитываемая один раз снаружи
  поверхности. Ширина — `min(310px, 88vw)`, ограниченная доступным пространством.
...
Оформление ограничено навигацией. `--sidebar-panel` равен `#f8f9f6` в светлой теме
и `#242722` в тёмной. Общая тень `--sidebar-panel-shadow`:

| Тема    | Тень                                                             |
| ------- | ---------------------------------------------------------------- |
| Светлая | `0 24px 52px rgb(39 50 26 / 19%), 0 5px 14px rgb(39 50 26 / 8%)` |
| Тёмная  | `0 24px 52px rgb(0 0 0 / 48%), 0 5px 14px rgb(0 0 0 / 26%)`      |

Локальные `--sidebar-surface`, `--sidebar-control-active`, `--sidebar-surface-shadow`
и `--sidebar-pressed-shadow` сохраняют палитру чата и прежнее нажатие.
Выбранные роли задают `--sidebar-button-rest-shadow`, а общий селектор управляет
hover, focus, disabled и active. Радиусы строки `18px` и меню `20px` не меняются.

<p>
  <a href="../apps/client/e2e/visual/__screenshots__/chromium/sidebar-floating-light.png"><img src="../apps/client/e2e/visual/__screenshots__/chromium/sidebar-floating-light.png" width="600" alt="Светлая парящая навигация рядом с чатом" /></a>
  <a href="../apps/client/e2e/visual/__screenshots__/chromium/sidebar-floating-dark.png"><img src="../apps/client/e2e/visual/__screenshots__/chromium/sidebar-floating-dark.png" width="600" alt="Тёмная парящая навигация и единый объём кнопок" /></a>
</p>
...
и его диалогами. Исключение — существующий `--chat-radius-surface: 28px`,
перенесённый в общие токены для подложки навигации. Значение и оформление чата
сохраняются; остальные примитивы не меняются:
...
| --color-sidebar        | `#eef0e9` | `#20231f` | Прежний фон; сейчас используется --sidebar-panel   |
...
Подложка карточки **совпадает** с подложкой пользовательского сообщения: `--chat-user`
(`#f1f3ed` / `#2d312a`). Не вводить для вставки отдельный оттенок поверхности.
