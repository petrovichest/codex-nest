...
- отправляет уведомления в браузере и Android, когда работа завершена, упала или
  требует внимания; и
- подключает вкладки Chrome к сессии, чтобы Codex мог читать браузер и управлять им.
...
- `apps/extension` — Chrome-расширение MV3 для управления браузером;
...
Отключение сохраняет вкладки, убирает их группу CodexNest и отсоединяет debugger.
Сервер сохраняет владельца привязки, поэтому повторно подключить её может только
тот же экземпляр расширения. В v1 нет переноса или восстановления после потери
данных профиля расширения.
...
Собрать папку для load-unpacked и детерминированный релизный ZIP:
